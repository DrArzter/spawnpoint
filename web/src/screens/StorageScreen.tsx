...
export function StorageScreen({ world, gameId, canReadBackups, canManageWorld, canRestoreBackup, onDownloadPack, onWorldAction, packRequest, lifecycleRequest }: {
  ...
  const backupColumns: DataColumn<BackupInventory["entries"][number]>[] = [
    ...
    { id: "checksum", label: "SHA-256", render: (row) => <code>{row.checksum.slice(0, 12)}…</code>, width: "0.9fr" },
    {
      ...
      render: (row) => <Button
        ...
        onClick={() => setConfirming({ action: "restore", backupKey: row.key, backupName: row.archiveName })}
        size="small"
        title={row.generationId === null ? "This legacy backup is not tied to a generation." : canRestoreBackup ? "Restore into a new generation" : "Your role cannot restore backups."}
      >Restore</Button>,
      width: "0.6fr",
    },
  ];
  ...
  return <>
    <PageHeader actions={world.worldLifecycleAvailable && world.materialization === "existing" && <>
      {world.materialization === "existing" && <Button disabled={!canManageWorld || lifecycleRequest.state === "pending"} onClick={() => setConfirming({ action: "archive" })}>Archive</Button>}
      {world.materialization === "existing" && <Button disabled={!canManageWorld || lifecycleRequest.state === "pending"} onClick={() => setConfirming({ action: "regenerate" })} variant="danger">Regenerate</Button>}
    ...
    {confirming !== null && <Surface aria-labelledby="world-operation-confirmation-title" className="operation-confirmation" role="alertdialog">
      <div><h2 id="world-operation-confirmation-title">{actionTitle}</h2><p>{actionDescription}</p></div>
      <div><Button onClick={() => setConfirming(null)} variant="ghost">Cancel</Button><Button onClick={confirmAction} variant={confirming.action === "archive" ? "primary" : "danger"}>{confirming.action === "archive" ? "Archive world" : confirming.action === "regenerate" ? "Create generation" : "Restore backup"}</Button></div>
    </Surface>}
