...
| Several worlds at once | A session is placed on a host with room ([ADR-0054](docs/adr/0054-place-a-session-on-a-host-with-room.md), the default since 2026-09-17): today two games share the configured instance, each in its own Compose project and slot; two worlds of one game still take turns, because a lifecycle record is one per game. Launching a host that fits when none has room is built and stays behind `launch = "disabled"` until the fleet bootstrap has run once; EC2 picks the instance type from the footprint's requirements, so no type or price lives in the code. The rollout and what is still owed are in [docs/capacity-allocation-rollout.md](docs/capacity-allocation-rollout.md) |
