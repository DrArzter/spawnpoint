...
| 11 — cutover | The access-api root's `placement` defaults to `shared`: every start the API or the bot requests is placed on a registered host with room, which today is the configured instance with another game beside it. `launch` stays `disabled` until the Parameter Store keys of the runbook exist. The Lambda code's own fallback for a missing `SPAWNPOINT_PLACEMENT` stays `single`, so a deployment that says nothing does what always ran | `placement = "single"` in the root's `terraform.tfvars` reverts it; sessions already running finish as they began. **Landed on 2026-09-17 at the owner's call, ahead of the phase-10 run; the acceptance report is still owed** |
...
Every phase but the run of phase 10 landed on 2026-09-17. The cutover (phase 11) went in ahead of the acceptance at the
owner's call: `placement` defaults to `shared`, `launch` stays `disabled`. What has never run against AWS, and must
before `launch` is enabled: the `CreateFleet` request as the start builds it, the fleet host's bootstrap end to end, the
Central API authorisation, and the host tier's discovery through the Docker socket on a real host. The first sessions
under `shared` on the owner's environment are the acceptance; its report belongs under `docs/acceptance/`, and
`placement = "single"` in the access-api tfvars is the revert if they disappoint.
