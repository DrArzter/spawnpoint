...
# GitHub snapshot adapter. A trusted workflow has already archived the exact
# commit into S3; CodeBuild verifies its identity and digest without executing
# anything from the source repository.

materialize_preset_source() {
  local workspace="$1"
  ...
  export CONFIG_SOURCE_ORIGIN="${CONFIG_REPOSITORY_URL%.git}"
  export CONFIG_SOURCE_REVISION="${CONFIG_COMMIT}"
}
