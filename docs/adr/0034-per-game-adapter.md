...
- Whether `game_query_players_raw` should also carry the stop-side "who is online" listing for notifications, or a
  count stays enough.
- ~~The factorio mod-portal resolver and how `mod-list.json` travels with a release payload.~~ Answered 2026-08-27:
  the resolver is `games/factorio/resolve-mods.sh` (public browse API, credentials only at download time — the server
  itself never needs a factorio.com account, since hidden servers skip matchmaking auth entirely), and `mod-list.json`
  travels nowhere: it is a pure function of the mod directory's contents, generated by the optional
  `game_prepare_session` hook at session start. The manifest schema stays mods-only.
