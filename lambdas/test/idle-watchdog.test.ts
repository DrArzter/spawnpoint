...
test("a failed probe never counts as empty", async () => {
  ...
  const failure = state(definition, "Count Probe Failure");
  ...
  const sendProbe = state(definition, "Send Probe");
  assert.equal(sendProbe.Catch?.[0]?.Next, "Count Probe Failure");
  const route = state(definition, "Route Probe Result");
...
test("players online resets the empty streak, and exit code 3 is the occupied signal", async () => {
  ...
  const counters = state(definition, "Session Occupied").Parameters?.counters as Record<string, unknown>;
  assert.equal(counters.empty, 0);
});
...
test("the stop is the verified stop workflow, run synchronously, and a refusal resumes watching", async () => {
  ...
  const refusalLimit = state(definition, "Stop Refusal Limit Reached");
...
test("an externally stopped host ends the watchdog successfully", async () => {
  const definition = await loadDefinition();
  const running = state(definition, "Host Still Running");
  assert.equal(running.Default, "Host Stopped Externally");
  const terminal = state(definition, "Host Stopped Externally");
...
test("blindness and repeated stop failure fail loudly instead of stopping the host", async () => {
  const definition = await loadDefinition();
  assert.equal(state(definition, "Watchdog Blind").Type, "Fail");
  assert.equal(state(definition, "Watchdog Stop Failing").Type, "Fail");
  assert.match(
    JSON.stringify(state(definition, "Watchdog Blind")),
    /left running/,
  );
});
