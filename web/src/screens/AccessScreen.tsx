...
function Users({ bootstrap, members, roles, onChange }: { bootstrap: OwnerBootstrap; members: Member[]; roles: Role[]; onChange: (members: Member[]) => void }) {
  ...
  const columns: DataColumn<Member>[] = [
    ...
    { id: "role", label: "Role", width: ".8fr", render: (member) => <select aria-label={`Role for ${member.name}`} onChange={(event) => onChange(members.map((item) => item.id === member.id ? { ...item, roleId: event.target.value } : item))} value={member.roleId}>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select> },
  ...
  return <div className="users-layout">
    <section className="bootstrap-owner">
      <div className="bootstrap-summary">
        <div><h2>Initial owner</h2><p>{bootstrap.state === "claimed" ? `${owner?.name ?? "The owner"} claimed the one-time setup with a verified Google account.` : `Sign in with the configured Google account (${bootstrap.email}) to create the first Owner.`}</p></div>
        <span className={`bootstrap-state ${bootstrap.state}`}>{bootstrap.state === "claimed" ? "Complete" : "Action required"}</span>
      </div>
      <dl><div><dt>Account</dt><dd>{bootstrap.email}</dd></div><div><dt>Role</dt><dd>Owner</dd></div>{bootstrap.state === "claimed" && <div><dt>Claimed</dt><dd>{bootstrap.claimedAt}</dd></div>}</dl>
      ...
      {showBootstrap && <ol className="bootstrap-steps"><li>Set the initial Owner email in the deployment configuration.</li><li>Sign in through Google with that exact verified email.</li><li>Spawnpoint creates the Owner and permanently closes the one-time setup.</li><li>Link Telegram, Discord and game accounts from the Owner profile.</li></ol>}
    </section>
...
function RoleForm({ onCreate, onCancel }: { onCreate: (role: Role) => void; onCancel: () => void }) {
  ...
  const [selected, setSelected] = useState<string[]>(["server.view"]);
...
function Notifications() {
  ...
  const columns: DataColumn<Game>[] = [
    { id: "game", label: "Game", render: (game) => <strong>{game.title}</strong>, width: "1fr" },
    { id: "started", label: "Started", render: (game) => <Setting checked={subscriptions[`${game.id}_started`]} label={`${game.title} started`} onChange={() => toggle(`${game.id}_started`)} />, width: "110px" },
    { id: "stopped", label: "Stopped", render: (game) => <Setting checked={subscriptions[`${game.id}_stopped`]} label={`${game.title} stopped`} onChange={() => toggle(`${game.id}_stopped`)} />, width: "110px" },
  ];
