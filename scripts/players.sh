...
remote_command='container_id=$(docker ps -q --filter label=com.docker.compose.service=mc); test -n "${container_id}" || { echo result=minecraft_not_running; exit 0; }; docker exec "${container_id}" rcon-cli list'
parameters="$(jq -cn --arg command "${remote_command}" '{commands:["set -euo pipefail",$command]}')"
command_id="$(
  aws ssm send-command \
    --instance-ids "${instance_id}" \
    --document-name AWS-RunShellScript \
    --comment 'Spawnpoint read-only player list' \
