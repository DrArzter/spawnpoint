...
check_terraform() {
  local root
  for root in infra/terraform infra/terraform-bootstrap infra/terraform-storage infra/terraform-guardrails infra/terraform-operations infra/terraform-releases infra/terraform-github; do
    [[ -d "${root}" ]] || continue
    printf -- '--- %s\n' "${root}"
