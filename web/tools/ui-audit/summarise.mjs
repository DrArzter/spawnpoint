...
function reportName(report) {
  if (report === "light") return "audit-light.json";
  if (report === "dark") return "audit-dark.json";
  throw new Error("Usage: node tools/ui-audit/summarise.mjs [light|dark]");
}

const probes = JSON.parse(readFileSync(resolve(reportName(process.argv[2] ?? "light")), "utf8"));
...
if (errors.length) print(`PROBE ERRORS: ${errors.length}`);
...
for (const [key, e] of [...fonts].sort((a, b) => b[1].count - a[1].count)) {
  ...
  print(`   ${key} (${e.count} nodes) e.g. ${[...e.samples].slice(0, 2).join(" | ")}`);
}
print(`   on-ramp steps in use: ${[...fonts].filter(([, e]) => e.onRamp).map(([k]) => k).join(", ")}`);
...
for (const [key, e] of [...radii].sort((a, b) => b[1].count - a[1].count)) {
  ...
  print(`   ${key} (${e.count}) e.g. ${[...e.samples].slice(0, 2).join(" | ")}`);
}
...
for (const [key, e] of [...zed].sort((a, b) => Number(b[0]) - Number(a[0]))) print(`   ${key}: ${[...e.samples].slice(0, 2).join(" | ")}`);
