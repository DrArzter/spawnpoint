...
test("desired is written before any host action, per ADR-0030", async () => {
  const definition = await loadDefinition();
  assert.equal(definition.States["Build Desired Document"].Next, "Write Desired");
  assert.equal(definition.States["Write Desired"].Next, "Describe Instance");
  // The stop and start of the host are reachable only after the pointer write.
  assert.equal(definition.States["Mark Origin Running"].Next, "Stop Session");
  assert.equal(definition.States["Mark Origin Stopped"].Next, "Start With Target");
});

test("active commits only after the target start succeeded", async () => {
  const definition = await loadDefinition();
  const start = definition.States["Start With Target"];
  ...
  assert.equal(definition.States["Build Commit Document"].Next, "Commit Active");

  const commit = definition.States["Commit Active"];
  const document = definition.States["Build Commit Document"].Parameters?.document as Record<string, unknown>;
...
test("rollback is the same mechanism in reverse: flip the pointer, start again", async () => {
  ...
  const gate = definition.States["Rollback Possible"];
  ...
  const rollbackDoc = definition.States["Build Rollback Document"].Parameters?.document as Record<string, unknown>;
  assert.equal(rollbackDoc["desired_release.$"], "$.pointer.active_release");

  assert.equal(definition.States["Write Rollback Desired"].Next, "Start With Previous");
  assert.equal(definition.States["Start With Previous"].Catch?.[0].Next, "Rollback Failed");
});

test("a refused stop restores the pointer before failing", async () => {
  const definition = await loadDefinition();
  assert.equal(definition.States["Stop Session"].Catch?.[0].Next, "Build Refusal Restore Document");

  const restored = definition.States["Build Refusal Restore Document"].Parameters?.document as Record<string, unknown>;
  assert.equal(restored["desired_release.$"], "$.pointer.desired_release");
  assert.equal(definition.States["Restore Desired After Refusal"].Next, "Promotion Refused");
  assert.equal(definition.States["Promotion Refused"].Type, "Fail");
});
...
test("the watchdog relaunch is fire-and-forget and never fails a finished promotion", async () => {
  ...
  for (const [name, terminal] of [
    ...
    const launch = definition.States[name];
    assert.equal(launch.Resource, "arn:aws:states:::aws-sdk:sfn:startExecution");
    assert.equal(launch.Catch?.[0].Next, terminal);
    assert.notEqual(definition.States[terminal].Type, "Fail");
  }
});

test("a promotion that cannot re-stop the host fails loudly, not silently", async () => {
  const definition = await loadDefinition();
  assert.equal(definition.States["Stop After Commit"].Catch?.[0].Next, "Promoted But Running");
  assert.equal(definition.States["Stop After Rollback"].Catch?.[0].Next, "Rolled Back But Running");
  for (const name of ["Promoted But Running", "Rolled Back But Running"]) {
    assert.equal(definition.States[name].Type, "Fail");
    assert.match(JSON.stringify(definition.States[name]), /running-hours alarm/);
  }
});

test("an already-active release is an idempotent success", async () => {
  const definition = await loadDefinition();
  const already = definition.States["Already Active"];
  ...
  assert.equal(definition.States["Nothing To Do"].End, true);
});
