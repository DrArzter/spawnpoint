...
Email and password is the second sign-in adapter ([ADR-0055](../../docs/adr/0055-sign-in-with-email-and-password-by-default.md)), on by default and switched off with `password_login_enabled = false`. `POST /auth/password/register` creates a credential — the address as a sign-in name, the password as a salted scrypt hash in the access table — and signs in; the account is observed like a Telegram one and holds no role until an Owner approves it. Nothing is sent to the address, so it is listed as unverified. `GET /auth/providers` tells the panel which adapters this deployment offers, so it draws only the buttons that lead somewhere. Ten wrong guesses lock an address for fifteen minutes.
